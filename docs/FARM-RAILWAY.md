...
**Full operations context:** [`FARM-OPERATIONS-HANDBOOK.md`](FARM-OPERATIONS-HANDBOOK.md) (parties, errors, monitoring).  
**CLI runbook:** [`CBTC-FARMING.md`](CBTC-FARMING.md).

Use a **separate git branch** (`cbtc-farming`) and **separate Railway service** from `oranjswap-web-mainnet`.
...
- `traffic rejection` — bot backs off automatically (see handbook §8)
- `Given holdings are invalid` — transient after 503/traffic; see handbook §8
- `npm error Class extends` on start — wrong start command; use Dockerfile CMD with `./node_modules/.bin/tsx`
