...
/**
 * Decode a JWT payload (base64url) WITHOUT verifying the signature. Safe here:
 * the token came straight from our trusted Authentik client-credentials flow
 * (getLedgerJwt) and never left the server. We only read claims, never trust
 * them for auth decisions.
 */
function decodeJwtPayload(jwt: string): Record<string, unknown> {
  const parts = jwt.split(".");
  if (parts.length < 2) throw new Error("Malformed JWT (no payload segment)");
  const json = Buffer.from(parts[1], "base64url").toString("utf8");
  return JSON.parse(json) as Record<string, unknown>;
}

/**
 * Resolve the Ledger-API user id that the m2m token authenticates as.
 * Canton's standard JWT auth maps the token `sub` claim to the ledger user.
 */
export async function getLedgerUserId(): Promise<string> {
  const jwt = await getLedgerJwt();
  const payload = decodeJwtPayload(jwt);
  const sub = payload.sub;
  if (typeof sub !== "string" || sub.length === 0) {
    throw new Error(
      "Ledger JWT has no `sub` claim — cannot determine the ledger user id to grant party rights to.",
    );
  }
  return sub;
}
...
/**
 * Grant the m2m ledger user CanActAs + CanReadAs rights for `party`.
 *
 * A freshly-allocated party is hosted on the participant, but the m2m user has
 * NO rights over it until we grant them — so every active-contracts read for
 * that party returns 403 PERMISSION_DENIED ("security-sensitive error").
 * This closes that gap right after allocation.
 *
 * Idempotent: re-granting an existing right is a no-op on Canton, so calling
 * this again for the same party is safe.
 */
export async function grantUserRightsForParty(party: string): Promise<void> {
  const userId = await getLedgerUserId();
  console.log(`${TAG} granting CanActAs+CanReadAs userId=${userId} party=${party.slice(0, 40)}...`);

  await ledgerFetch<unknown>(
  ...
  console.log(`${TAG} rights granted for party=${party.slice(0, 40)}...`);
}

/**
 * List the current rights of the m2m ledger user. Diagnostic only — used to
 * confirm whether the token has ParticipantAdmin (required to grant rights).
 * Returns the raw response from GET /v2/users/{user-id}/rights.
 */
export async function listSelfUserRights(): Promise<{
  userId: string;
  rights: unknown;
}> {
  const userId = await getLedgerUserId();
  const rights = await ledgerFetch<unknown>(
    `/v2/users/${encodeURIComponent(userId)}/rights`,
    { method: "GET" },
  );
