/**
 * POST /api/htlc/prepare-lock-intent — reverse Loop: reserve WBTC float, then build CBTC transfer.
 */
...
export async function POST(req: Request) {
  try {
    ...
    const rateLimit = await requireHtlcPrepareRateLimit(auth.partyId);
    ...
    const out = await htlcService().prepareReverseLoopLockIntent(
      {
        id,
        direction,
