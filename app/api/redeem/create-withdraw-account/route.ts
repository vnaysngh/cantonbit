...
export async function POST(req: NextRequest) {
  ...
  try {
    const { partyId: clientPartyId, destinationBtcAddress } =
      (await req.json()) as {
        partyId?: string;
    ...
    console.log(
      `${TAG} partyId=${partyId.slice(0, 30)}... btcAddress=${destinationBtcAddress}`
    );
    console.log(
    ...
    console.log(
      `${TAG} wa_rules createdEventBlob length=${rules.wa_rules.created_event_blob?.length ?? 0}`
    );
    ...
    console.log(
      `${TAG} success! withdrawAccount contractId=${account.contractId}`
    );
    console.log(
      `${TAG} createdEventBlob from tx length=${account.createdEventBlob.length}`
    );
    ...
    {
      console.log(
        `${TAG} fetching authoritative templateId+blob from active-contracts...`
      );
      try {
        const ledgerEndRes = await fetch(
          `${NETWORK.ledgerHost}/v2/state/ledger-end`,
          {
        ...
        const acsRes = await fetch(
          `${NETWORK.ledgerHost}/v2/state/active-contracts`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        for (const entry of entries) {
          ...
          if (ev?.contractId === account.contractId) {
            ...
            console.log(
              `${TAG} authoritative from ACS: templateId=${account.templateId} blobLen=${account.createdEventBlob.length}`
            );
            break;
          }
        }
        ...
        console.warn(
          `${TAG} failed to fetch authoritative templateId/blob from ACS:`,
          blobErr
        );
      }
    }
    ...
    console.log(
      `${TAG} returned.createdEventBlob.length=${account.createdEventBlob.length}`
    );
    if (!account.templateId) {
      console.error(
        `${TAG} WARNING: templateId is EMPTY — the burn will reject (submit-withdraw now requires it)`
      );
    }
    console.log(`${TAG} ===========================`);
...
function extractWithdrawAccount(
  data: unknown
): { contractId: string; templateId: string; createdEventBlob: string } | null {
  ...
  if (txn?.eventsById) {
    for (const ev of Object.values(txn.eventsById)) {
      ...
      if (
        created?.contractId &&
        created.templateId?.includes("CBTCWithdrawAccount")
      ) {
        console.log(
          `[redeem/create-withdraw-account] found via transaction.eventsById (template match)`
        );
        return {
    ...
    for (const ev of Object.values(txn.eventsById)) {
      ...
      if (created?.contractId) {
        console.log(
          `[redeem/create-withdraw-account] found via transaction.eventsById (first created)`
        );
        return {
          contractId: created.contractId,
          templateId: created.templateId ?? "",
  ...
  if (tree?.eventsById) {
    for (const ev of Object.values(tree.eventsById)) {
      ...
      if (
        val?.contractId &&
        val.templateId?.includes("CBTCWithdrawAccount") &&
        !val.templateId.includes("Rules")
      ) {
        console.log(
          `[redeem/create-withdraw-account] found via CreatedTreeEvent.value (template match)`
        );
        return {
          contractId: val.contractId,
          templateId: val.templateId,
          createdEventBlob: val.createdEventBlob ?? ""
        };
      }
    }
    ...
    for (const ev of Object.values(tree.eventsById)) {
      ...
      if (val?.contractId) {
        console.log(
          `[redeem/create-withdraw-account] found via CreatedTreeEvent.value (first created)`
        );
        return {
          contractId: val.contractId,
          templateId: val.templateId ?? "",
    ...
    for (const ev of Object.values(tree.eventsById)) {
      const exercised = (
        ev as {
          ExercisedTreeEvent?: {
            value?: { exerciseResult?: { withdrawAccountCid?: string } };
          };
        }
      ).ExercisedTreeEvent?.value;
      if (exercised?.exerciseResult?.withdrawAccountCid) {
        console.log(
          `[redeem/create-withdraw-account] found via ExercisedTreeEvent.exerciseResult.withdrawAccountCid`
        );
        return {
          contractId: exercised.exerciseResult.withdrawAccountCid,
          templateId: "",
          createdEventBlob: ""
        };
      }
