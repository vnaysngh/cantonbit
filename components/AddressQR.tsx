...
export function AddressQR({ value, label, size = 240, className }: Props) {
  ...
  return (
    <div className={cn("flex w-full min-w-0 flex-col items-stretch gap-6", className)}>
      {/* QR */}
      ...
      <div className="w-full min-w-0 max-w-xl space-y-3 self-stretch">
      ...
      {label && (
        <div className="w-full min-w-0 text-center text-xs leading-relaxed text-muted-foreground">
          {label}
        </div>
      )}
    </div>
