...
if (!label) {
  console.error("usage: node scripts/snapshot.mjs <label>");
  process.exit(1);
}
...
const WARPX =
  "warpx-mainnet-1::1220517bfd86ef5732610705a35b7b2d56e36112550d6a2b778971dbd099a3d36e99";
const USER =
...
async function acs(party) {
  const r = await fetch(`${LEDGER}/v2/state/active-contracts`, {
    method: "POST",
    headers: {
...
function holdings(raw) {
  ...
  for (const i of Array.isArray(raw) ? raw : []) {
    ...
    if (ev?.templateId?.includes("Holding.V0.Holding:Holding"))
      out.push({
        amount: ev.createArgument?.amount,
...
console.log(
  `  warpx: ${snapshot.summary.warpxTotal} CBTC (${snapshot.summary.warpxHoldings.length} holdings)`
);
console.log(
  `  user:  ${snapshot.summary.userTotal} CBTC (${snapshot.summary.userHoldings.length} holdings)`
);
