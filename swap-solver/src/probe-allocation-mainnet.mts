...
async function main() {
  ...
  console.log(
    `[0] float before: ${Number(floatBefore) / 1e8} CBTC (${floatBefore} sats)`
  );
  ...
  try {
    ...
    if (MODE === "happy") {
      ...
      const { updateId } = await canton.executeAllocation(
        allocationCid,
        RECIPIENT
      );
      executed = true;
      ...
      console.log(
        `    (0.00001 CBTC now at recipient — recover from that wallet)`
      );
    } else {
      ...
      executed = true;
      console.log(
        `    ✓ withdraw submitted. updateId=${updateId.slice(0, 24)}…`
      );
      await sleep(2000);
    ...
    if (!executed) {
      ...
      try {
        ...
        console.error(
          `[safety] ✗ withdraw FAILED — MANUAL RECOVERY NEEDED for ${allocationCid}: ${e instanceof Error ? e.message : e}`
        );
      }
    }
  }
