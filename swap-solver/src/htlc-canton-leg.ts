...
export async function releaseCbtcOnReveal(
  ...
  if (!preimageMatches(reveal.preimageHex, reveal.hashLock)) {
    return {
      kind: "failed",
      reason: "preimage does not match hashLock — refusing to release CBTC"
    };
  }
  ...
  try {
    ...
    return {
      kind: "skipped",
      reason: `holdings read failed (transient): ${errMsg(e)}`
    };
  }
  try {
    const { updateId, offerContractId, autoAccepted } =
    ...
    return {
      kind: "failed",
      reason: `release (createOffer) failed: ${errMsg(e)}`
    };
  }
}
