...
async function main(): Promise<void> {
  ...
  switch (cmd) {
    case "status": {
      const r = await buildHealthReport(store, {
        now: now(),
      ...
      if (r.atRisk.length) {
        console.log(
          `⚠ AT RISK (CBTC delivered, WBTC not yet finalised) — solver capital exposed:`
        );
        for (const o of r.atRisk)
          console.log(
            `   ${o.orderId.slice(0, 16)}…  status=${o.status}  deadline_in=${o.order.fillDeadline - now()}s`
          );
      }
      if (r.stuckDelivering.length) {
        ...
        for (const o of r.stuckDelivering)
          console.log(
            `   ${o.orderId.slice(0, 16)}…  deadline_in=${o.order.fillDeadline - now()}s  ref=${o.cantonDeliveryRef ?? "-"}`
          );
      }
      if (r.staleSeen.length) {
        console.log(`\n⚠ STALE SEEN (locked, delivery not started):`);
        for (const o of r.staleSeen)
          console.log(`   ${o.orderId.slice(0, 16)}…  seenAt=${o.createdAt}`);
      }
      if (r.failed.length) {
        console.log(`\n✗ FAILED (needs attention):`);
        for (const o of r.failed)
          console.log(`   ${o.orderId.slice(0, 16)}…  ${o.note ?? ""}`);
      }
      if (r.reconciliationGaps.length) {
        console.log(`\n✗ RECONCILIATION GAPS (missing tx/timestamp records):`);
        for (const o of r.reconciliationGaps)
          console.log(`   ${o.orderId.slice(0, 16)}…  status=${o.status}`);
      }
      if (r.status === "ok")
        console.log("All clear — no orders need attention.");
      console.log("");
    ...
    case "list": {
      const orders = (await allOrders(store)).filter(
        (o) => !arg || o.status === arg
      );
      ...
      for (const o of orders) {
        console.log(
          `  ${o.orderId.slice(0, 18)}…  ${o.status.padEnd(11)}  fill=${o.fillTimestamp ?? "-"}  ${o.note ?? ""}`
        );
      }
      console.log("");
      break;
    ...
    case "show": {
      if (!arg) {
        console.error("usage: show <orderId>");
        process.exit(1);
      }
      const rec =
        (await store.get(arg as Hex)) ??
        (await allOrders(store)).find((o) => o.orderId.startsWith(arg!));
      if (!rec) {
