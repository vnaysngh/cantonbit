...
async function main() {
  ...
  console.log(`[2] solver locks ${WBTC_AMOUNT} WBTC on EVM under H (receiver = user)…`);
  const a = (await wbtc.read.allowance([account.address, ESCROW])) as bigint;
  if (a < WBTC_AMOUNT) {
    const apv = await wbtc.write.approve([ESCROW, WBTC_AMOUNT * 10n], { account, chain: null });
    await pub.waitForTransactionReceipt({ hash: apv });
  }
  const lockTx = await escrow.write.lock([hashLock, BigInt(tl.userTimelock), WBTC_AMOUNT, WBTC, userEvm], { account, chain: null });
  ...
  if (rel.kind !== "released") throw new Error(`cBTC transfer failed: ${JSON.stringify(rel)}`);
  ...
  if (revealed?.toLowerCase() !== secret.toLowerCase()) throw new Error("solver could not read revealed s");
  console.log(`[5] ✓ solver read the revealed preimage from chain (already holds the cBTC).`);
...
main().catch((e) => { console.error("\n[e2e-reverse] FAILED:", e instanceof Error ? e.message : e); process.exit(1); });
