...
const CLIENT_ID =
  process.env.KEYCLOAK_CLIENT_ID_DEVNET ?? "validator-devnet-m2m";
...
async function main() {
  ...
  console.log(
    `receiver (Loop, x-participant): ${LOOP_RECEIVER.slice(0, 30)}…\n`
  );
  ...
  const now = Date.now();
  console.log(
    `[1] allocating ${AMOUNT} CBTC (executor=sender=us, receiver=Loop)…`
  );
  const { allocationCid } = await canton.allocate({
    receiverParty: LOOP_RECEIVER,
    amountBtc: AMOUNT,
  ...
  try {
    ...
    console.log(
      `    update ${updateId.slice(0, 20)}… → Shape B (fully-trustless Loop swap) is POSSIBLE.`
    );
    await sleep(2000);
    const float1 = await canton.getFloatSats();
    console.log(
      `[3] float after: ${Number(float1) / 1e8} CBTC (down by ${AMOUNT} → CBTC left to Loop).`
    );
  } catch (e) {
    ...
    if (/AUTHORIZATION/i.test(msg)) {
      console.error(
        `    → DAML_AUTHORIZATION_ERROR: the registry DEMANDS receiver co-auth.`
      );
      console.error(
        `    → For a x-participant Loop receiver we CAN'T provide it → Shape B IMPOSSIBLE.`
      );
      console.error(
        `    → The standard transfer-accept flow (Loop user accepts) would be required.`
      );
    }
  } finally {
    if (!done) {
      ...
      try {
        ...
        console.error(`[safety] ✓ recovered (${updateId.slice(0, 16)}…).`);
      } catch (e) {
        console.error(
