...
function key(): Hex {
  const r = env("PRIVATE_KEY");
  return (r.startsWith("0x") ? r : `0x${r}`) as Hex;
}
function art(p: string): { abi: unknown[]; bytecode: Hex } {
  const j = JSON.parse(readFileSync(`../contracts/out/${p}`, "utf8"));
...
async function main() {
  ...
  const waitBal = async (who: Address, want: bigint, label: string) => {
    for (let i = 0; i < 25; i++) {
      if ((await balOf(who)) === want) return;
      await sleep(1500);
    }
    throw new Error(`${label}: ${who} balance never reached ${want}`);
  };
  ...
  if (allowance < 10n ** 18n)
    await pub.waitForTransactionReceipt({
      hash: await getContract({
        ...
        client: wallet
      }).write.approve([PERMIT2_ADDRESS, 2n ** 256n - 1n])
    });
  ...
  if ((await balOf(account.address)) < WBTC_LOCK)
    await pub.waitForTransactionReceipt({
      hash: await getContract({
        ...
        client: wallet
      }).write.mint([account.address, WBTC_LOCK])
    });
    ...
    outputs: [
      {
        ...
        chainId: 1_000_000_000_000_001n,
  ...
  const storePath = "/tmp/oranj-e2e-full.json";
  rmSync(storePath, { force: true });
  const store = new InMemoryOrderStore();
  await new OpenWatcher(
    { rpcUrl: RPC, escrow, startBlock },
    store,
    () => {}
  ).backfill();
  ...
  console.log(
    autoAccepted ? `(auto-accepted by wallet)` : `offerId: ${offerContractId}`
  );
  ...
  if (autoAccepted) {
    ...
    console.log(
      `>>> ACCEPT the incoming 0.0001 CBTC transfer in your wallet now <<<\n`
    );
    let resolved = false;
    fillTimestamp = now;
    for (let i = 0; i < 120; i++) {
      const active = await canton
        .isOfferActive(RECIPIENT, offerContractId)
        .catch(() => true);
      if (!active) {
        const r = await canton.resolveOffer({
          receiverParty: RECIPIENT,
          offerContractId,
          fromOffset: 0
        });
        if (r.kind === "accepted") {
          fillTimestamp = recordTimeToUnixSeconds(r.recordTime);
          console.log(`\naccepted at ${r.recordTime} ✓`);
          summary["5. accept record-time"] = r.recordTime;
          resolved = true;
          break;
        }
        if (r.kind === "expired") throw new Error("offer expired unaccepted");
      }
      process.stdout.write(`  waiting… (${i * 5}s)\r`);
      await sleep(5000);
    }
  ...
  assert.equal(
    outcome.kind,
    "finalised",
    `settle failed: ${JSON.stringify(outcome)}`
  );
  await waitBal(account.address, userBefore + WBTC_LOCK, "release");
  console.log(`WBTC released ✓`);
  if (outcome.kind === "finalised") {
    summary["6. attest (Base)"] = outcome.attestTxHash ?? "-";
    summary["6. finalise (Base)"] = outcome.finaliseTxHash ?? "-";
  }

  // === RESULT ===
  console.log(`\n\n========== ✓ FULL CROSS-CHAIN SWAP PASSED ==========`);
  console.log(
    `  ${WBTC_LOCK} WBTC (Base Sepolia) → ${CBTC_BTC} CBTC (Canton DevNet)`
  );
...
main().catch((e) => {
  console.error("\n✗ FULL E2E FAILED:", e instanceof Error ? e.message : e);
  process.exit(1);
});
