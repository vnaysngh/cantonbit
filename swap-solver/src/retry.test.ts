...
test("isTransientError: does NOT retry deterministic failures", () => {
  ...
  assert.ok(!isTransientError(new Error("Canton party does not match the recipient")));
...
test("retry: gives up after max retries and throws the last error", async () => {
  ...
  await assert.rejects(
    retry(async () => { calls++; throw new Error("timeout"); }, { retries: 2, baseMs: 1, maxMs: 2 }),
    /timeout/,
  );
...
test("retry: does NOT retry a deterministic error (fails fast)", async () => {
  let calls = 0;
  await assert.rejects(
    retry(async () => { calls++; throw new Error("NotProven"); }, { retries: 5, baseMs: 1 }),
    /NotProven/,
  );
...
test("retry: returns immediately on first success", async () => {
  let calls = 0;
  const r = await retry(async () => { calls++; return 42; });
  assert.equal(r, 42);
