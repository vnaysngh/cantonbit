...
export async function buildHealthReport(
  ...
  const atRisk = all.filter(
    (o) => o.status === "delivered" || o.status === "attested"
  );

  const stuckDelivering = all.filter(
    (o) =>
      o.status === "delivering" &&
      o.order.fillDeadline - p.now < p.deadlineWarnSeconds
  );

  const staleSeen = all.filter(
    (o) =>
      o.status === "seen" &&
      p.now - Math.floor(Date.parse(o.createdAt) / 1000) > p.staleSeenSeconds
  );
  ...
  const reconciliationGaps = all.filter((o) => {
    ...
    if (
      (o.status === "delivered" || o.status === "attested") &&
      o.fillTimestamp == null
    )
      return true;
    return false;
  });
  ...
  if (staleSeen.length || stuckDelivering.length || failed.length)
    status = "warn";
...
export function summarize(r: HealthReport): string {
  ...
  const flags = [
    ...
    r.stuckDelivering.length
      ? `stuckDelivering:${r.stuckDelivering.length}`
      : "",
    r.staleSeen.length ? `staleSeen:${r.staleSeen.length}` : "",
    r.failed.length ? `failed:${r.failed.length}` : "",
    r.reconciliationGaps.length
      ? `reconGaps:${r.reconciliationGaps.length}`
      : ""
  ]
    .filter(Boolean)
    .join(" ");
