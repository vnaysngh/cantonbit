...
const SOLVER_EVM =
  process.env.SOLVER_EVM ?? "0x0B95ec21579aee6Ef7b712976bD86689D68b5A08";
...
async function main() {
  if (!RECV)
    throw new Error("set RECV_PARTY to an onboarded participant-managed party");
  ...
  await jpost("/api/htlc", {
    ...
    hashLock: H,
    userEvmAddress: user.address,
    solverEvmAddress: SOLVER_EVM,
    wbtcAmount: wbtcUnits.toString(),
    userTimelock: now + 14400,
    userCantonParty: RECV,
    solverCantonParty: SOLVER_CANTON,
    cbtcAmount: "0.0001",
    solverTimelock: now + 10800
  });
  ...
  const wbtc = getContract({
    address: WBTC,
    abi: ERC20,
    client: { public: pub, wallet: w }
  });
  const apv = await wbtc.write.approve([ESCROW, wbtcUnits * 5n], {
    account: user,
    chain: null
  });
  ...
  const lockTx = await escrow.write.lock(
    [H, BigInt(now + 14400), wbtcUnits, WBTC, SOLVER_EVM as Address],
  ...
  console.log(
    `[2] WBTC locked on-chain (tx ${lockTx.slice(0, 14)}…) → status main_locked`
  );
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (order?.status === "counter_locked") {
      locked = true;
      console.log("    ✓ daemon locked the CBTC counter (allocate + HtlcLock)");
      break;
    }
    if (
  ...
  if (!locked)
    throw new Error(
      "daemon did not lock the counter — is the daemon running with R3 code?"
    );

  // 4. claim-managed (backend signs the CBTC claim via CanActAs — the user's "Claim")
  console.log(
    "[4] claim-managed: backend claims CBTC for the user (CanActAs)…"
  );
  const claim = await jpost(`/api/htlc/${id}/claim-managed`, { preimage: PRE });
  console.log(
    `    ✓ CBTC claimed on-ledger (updateId ${String(claim.updateId).slice(0, 14)}…)`
  );
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (order?.status === "main_claimed") {
      ...
      console.log(
        `    ✓ daemon claimed WBTC (tx ${String(order.mainClaimTx).slice(0, 14)}…)`
      );
      break;
    }
  }
  ...
  console.log(
    `   Daemon: locked CBTC + claimed WBTC. Backend: claimed CBTC for the user. CBTC delivered to ${RECV.slice(0, 20)}….`
  );
}

main().catch((e) => {
  console.error("\n[r3-e2e] FAILED:", e instanceof Error ? e.message : e);
  process.exit(1);
});
