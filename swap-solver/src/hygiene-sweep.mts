...
async function main() {
  ...
  console.log(
    `  scanned ${Number(tip - startBlock)} blocks, ${logs.length} Locked event(s)`
  );
  ...
  for (const log of logs) {
    const a = (log as { args?: { hashValue?: Hex; senderAddress?: Address } })
      .args;
    if (!a?.hashValue || seen.has(a.hashValue)) continue;
    seen.add(a.hashValue);
    if (a.senderAddress?.toLowerCase() !== account.address.toLowerCase()) {
      skipped++;
      continue;
    }
    const lock = (await escrow.read.locks([a.hashValue])) as readonly [
      bigint,
      bigint,
      Address,
      Address,
    ...
    if (Number(lock[0]) > now) {
      console.log(
        `  ${a.hashValue.slice(0, 14)}… still timelocked (until ${lock[0]}) — skip`
      );
      continue;
    }
    console.log(`  retaking ${a.hashValue.slice(0, 14)}… amount=${lock[1]}`);
    const tx = await escrow.write.retake([a.hashValue], {
      account,
      chain: null
    });
  ...
  console.log(
    `  ✓ retook ${retaken} lock(s); ${skipped} foreign-sender events ignored.`
  );

  // ---- 2. Canton: refund expired counter-locked orders + orphan allocations ----
  const headers = API_AUTH_TOKEN
    ? { Authorization: `Bearer ${API_AUTH_TOKEN}` }
    : undefined;
  console.log(`[2] auto-refund expired swaps via API…`);
  const r1 = await fetch(`${API_BASE}/api/htlc/auto-refund`, {
    method: "POST",
    headers
  });
  console.log(`  auto-refund:`, JSON.stringify(await r1.json()).slice(0, 200));
  console.log(`[3] cleanup orphan allocations…`);
  const r2 = await fetch(`${API_BASE}/api/htlc/cleanup-allocations`, {
    method: "POST",
    headers
  });
  console.log(
    `  cleanup-allocations:`,
    JSON.stringify(await r2.json()).slice(0, 300)
  );
  console.log(`\nDONE.`);
}

main().catch((e) => {
  console.error("[hygiene] FATAL:", e instanceof Error ? e.message : e);
  process.exit(1);
});
