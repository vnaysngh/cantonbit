...
async function main() {
  ...
  log(
    `1. Synthesise a 'seen' order asking ${Number(askSats) / 1e8} CBTC (> float)`
  );
  ...
  assert.equal(
    (await store.get(orderId))?.status,
    "seen",
    "setup: order should be seen"
  );

  log("2. Run the REAL startDelivery guard");
  const outcome = await startDelivery(store, canton, orderId, {
    now,
    minSecondsBeforeDeadline: 30 * 60,
    cbtcDecimals: CBTC_DECIMALS
  });
  ...
  assert.equal(
    outcome.kind,
    "failed",
    `expected 'failed', got '${outcome.kind}'`
  );
  assert.ok(
    /insufficient CBTC float/i.test((outcome as { reason: string }).reason),
    `reason should mention insufficient float, got: ${(outcome as { reason: string }).reason}`
  );
  const rec = (await store.get(orderId))!;
  assert.equal(
    rec.status,
    "failed",
    `store status should be 'failed', got '${rec.status}'`
  );
  assert.ok(
    !rec.cantonDeliveryRef,
    "NO offer should have been created (cantonDeliveryRef must be empty)"
  );
  ...
  assert.equal(
    floatAfter,
    floatSats,
    `float changed! before=${floatSats} after=${floatAfter}`
  );
  ...
  console.log(
    `  asked ${Number(askSats) / 1e8} CBTC vs ${Number(floatSats) / 1e8} float → refused, no delivery, float intact`
  );
  console.log(`=====================================================`);
}
