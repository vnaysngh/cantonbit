...
function scan(label: string, blob: unknown): string[] {
  ...
  console.log(
    `  [${label}] hashlock-keyword hits: ${hits.length ? hits.join(", ") : "NONE"}`
  );
  return hits;
}
...
async function main() {
  ...
  console.log(
    `admin: ${ADMIN.slice(0, 28)}…   solver: ${SOLVER.slice(0, 28)}…\n`
  );
  ...
  try {
    ...
    const supported =
      (j as any)?.instruments?.[0]?.supportedApis ??
      (j as any)?.supportedApis ??
      j;
    console.log(`    supportedApis: ${JSON.stringify(supported)}`);
    allHits.push(...scan("metadata", j));
  } catch (e) {
    console.log(
      `    metadata fetch error: ${e instanceof Error ? e.message : e}`
    );
  }
  ...
  try {
    ...
    if (holdings.length) {
      ...
      console.log(
        `    sample holding cid: ${String(h.contractId ?? h.cid ?? "?").slice(0, 24)}…`
      );
      ...
      console.log(
        `    -> NOTE: there is NO 'hash'/'preimage'/'condition' field. The only`
      );
      console.log(
        `       conditions a lock can express are TIME (expiresAt/expiresAfter).`
      );
      console.log(
        `       'context' is a free-text tag, NOT an enforced unlock condition.`
      );
      allHits.push(...scan("holding-lock", h.lock ?? {}));
    } else {
      console.log(
        `    (no holdings on solver party — lock-shape read skipped; metadata still decisive)`
      );
    }
  } catch (e) {
    console.log(
      `    holdings read error: ${e instanceof Error ? e.message : e}`
    );
  }
  ...
  if (uniqueHits.length === 0) {
    ...
    console.log(
      `  • registry exposes only transfer-factory + allocation-factory`
    );
    console.log(`  • no htlc/hashlock/conditional factory endpoint exists`);
    console.log(
      `=> CONFIRMED: a 1inch-style HTLC atomic swap is NOT buildable for ${INSTRUMENT}.`
    );
    console.log(
      `   Reachable ceiling = Allocation/TimeLock lock + slashable solver bond.`
    );
  } else {
    console.log(
      `POSSIBLE hashlock-related surface found: ${uniqueHits.join(", ")}`
    );
    console.log(
      `=> INVESTIGATE: a preimage condition may be expressible. HTLC might be on the table.`
    );
  }
