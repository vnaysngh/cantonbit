...
test("indefinite lock (no expiry fields) → locked", () => {
  ...
  assert.equal(
    isActivelyLocked({ expiresAt: null, expiresAfter: null }, NOW),
    true
  );
...
test("lock expiring in the FUTURE → still locked", () => {
  assert.equal(
    isActivelyLocked({ expiresAt: "2026-06-05T13:00:00.000Z" }, NOW),
    true
  );
});
...
test("lock that already EXPIRED → spendable (registry allows expired-lock inputs)", () => {
  assert.equal(
    isActivelyLocked({ expiresAt: "2026-06-05T11:00:00.000Z" }, NOW),
    false
  );
});
...
test("relative expiry (expiresAfter) we can't resolve → conservatively locked", () => {
  assert.equal(
    isActivelyLocked({ expiresAfter: "some-relative-ref" }, NOW),
    true
  );
});
