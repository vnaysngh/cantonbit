...
function expectApiError(fn: () => unknown, status: number, match: RegExp) {
  try {
    ...
    assert.equal(
      e.status,
      status,
      `status should be ${status}, got ${e.status}`
    );
...
test("GATE A: wrong INPUT token is rejected (not the configured WBTC)", () => {
  ...
  expectApiError(
    () => validateOrderIntake(bad, out0, cfg),
    400,
    /input token is not the configured WBTC/
  );
});
...
test("GATE A: wrong OUTPUT token is rejected (not the configured CBTC)", () => {
  ...
  expectApiError(
    () => validateOrderIntake(order, bad, cfg),
    400,
    /output token is not the configured CBTC/
  );
});
...
test("GATE A: WBTC match is case-insensitive (checksum vs lowercase)", () => {
  ...
  const amt = validateOrderIntake(order, out0, {
    ...cfg,
...
test("GATE B: a banned user is rejected (403)", () => {
  ...
  expectApiError(
    () => validateOrderIntake(order, out0, { ...cfg, bannedUsers: banned }),
    403,
    /not permitted/
  );
});
...
test("GATE B: banned check is case-insensitive", () => {
  ...
  expectApiError(
    () => validateOrderIntake(upper, out0, { ...cfg, bannedUsers: banned }),
    403,
    /not permitted/
  );
});
...
test("GATE C: the largest legal amount (uint256 - 1) passes", () => {
  ...
  const order = {
    user: USER as string,
    inputs: [[BigInt(WBTC), max]] as const
  };
...
test("GATE D: a REAL signature over our typed data recovers to the signer", async () => {
  ...
  assert.equal(
    ...
    "the gate's recover must return the true signer"
  );
  assert.notEqual(
    getAddress(recovered),
    getAddress(USER),
    "a DIFFERENT user would not match → gate would reject"
  );
});
