...
async function main() {
  ...
  if (solver.address.toLowerCase() === user.address.toLowerCase())
    throw new Error("USER and SOLVER must be different wallets");
  ...
  console.log(
    `Canton receiver (user party): ${USER_CANTON_PARTY.slice(0, 24)}…\n`
  );

  const wbtcUser = getContract({
    address: WBTC,
    abi: ERC20,
    client: { public: pub, wallet: userWallet }
  });
  const escrowUser = getContract({
    address: ESCROW,
    abi: HTLC_ESCROW_ABI,
    client: { public: pub, wallet: userWallet }
  });
  ...
  // balances BEFORE
  const solverWbtc0 = (await wbtcUser.read.balanceOf([
    solver.address
  ])) as bigint;
  const userWbtc0 = (await wbtcUser.read.balanceOf([user.address])) as bigint;
  console.log(
    `[balances before] USER WBTC=${formatUnits(userWbtc0, 8)}  SOLVER WBTC=${formatUnits(solverWbtc0, 8)}`
  );
  if (userWbtc0 < WBTC_AMOUNT)
    throw new Error(`USER needs >= ${formatUnits(WBTC_AMOUNT, 8)} WBTC`);
  ...
  console.log(
    `[2] USER locks ${formatUnits(WBTC_AMOUNT, 8)} WBTC, receiver=SOLVER…`
  );
  const apv = await wbtcUser.write.approve([ESCROW, WBTC_AMOUNT * 5n], {
    account: user,
    chain: null
  });
  await pub.waitForTransactionReceipt({ hash: apv });
  const lockTx = await escrowUser.write.lock(
    [hashLock, BigInt(tl.userTimelock), WBTC_AMOUNT, WBTC, solver.address],
    { account: user, chain: null }
  );
  ...
  for (let i = 0; i < 8; i++) {
    lock = (await escrowUser.read.locks([hashLock])) as readonly [
      bigint,
      bigint,
      Address,
      Address,
  ...
  if (!lock || lock[1] !== WBTC_AMOUNT)
    throw new Error("lock not visible on EVM");
  if (lock[4].toLowerCase() !== solver.address.toLowerCase())
    throw new Error("lock receiver != SOLVER");
  console.log(
    `[3] SOLVER confirmed lock (receiver=SOLVER ✓, amount=${formatUnits(lock[1], 8)}).`
  );
  ...
  console.log(
    `[5] SOLVER delivers ${CBTC_AMOUNT} CBTC to USER on Canton (gated on preimage)…`
  );
  const rel = await releaseCbtcOnReveal(
    canton,
    {
      receiverParty: USER_CANTON_PARTY,
  ...
  if (claim.kind !== "claimed")
    throw new Error(`SOLVER claim failed: ${JSON.stringify(claim)}`);
  console.log(`    ✓ claimed. tx=${claim.txHash.slice(0, 18)}…`);

  // 7. verify the WBTC actually moved USER → SOLVER
  await sleep(3000);
  const solverWbtc1 = (await wbtcUser.read.balanceOf([
    solver.address
  ])) as bigint;
  ...
  console.log(
    `USER  WBTC delta: -${formatUnits(userDelta, 8)}  (expect -${formatUnits(WBTC_AMOUNT, 8)})`
  );
  console.log(
    `SOLVER WBTC delta: +${formatUnits(solverDelta, 8)}  (expect +${formatUnits(WBTC_AMOUNT, 8)})`
  );

  if (userDelta !== WBTC_AMOUNT)
    throw new Error("USER WBTC did not decrease by the swap amount");
  if (solverDelta !== WBTC_AMOUNT)
    throw new Error("SOLVER WBTC did not increase by the swap amount");

  console.log(`\n✅✅ REAL 2-PARTY EVM→Canton swap COMPLETE:`);
  console.log(
    `   WBTC moved USER → SOLVER (${formatUnits(WBTC_AMOUNT, 8)}), CBTC moved SOLVER → USER (${CBTC_AMOUNT}).`
  );
}

main().catch((e) => {
  console.error("\n[2party-e2e] FAILED:", e instanceof Error ? e.message : e);
  process.exit(1);
});
