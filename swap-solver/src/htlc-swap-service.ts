...
export class InMemorySwapStore implements SwapStore {
  ...
  async put(o: SwapOrder) { this.m.set(o.id, o); }
  async byStatus(s: SwapStatus) { return [...this.m.values()].filter((o) => o.status === s); }
}
...
export class HtlcSwapService {
  constructor(private store: SwapStore, private canton: CantonClient) {}
  ...
  async accept(id: string): Promise<SwapOrder> {
    ...
    if (o.status !== "open") throw new Error(`order ${id} not open (status ${o.status})`);
    o.status = "accepted";
  ...
  async recordMainLock(id: string, mainLockTx: Hex): Promise<SwapOrder> {
    const o = await this.must(id);
    if (o.status !== "accepted") throw new Error(`order ${id} not accepted (status ${o.status})`);
  ...
  async lockCounter(id: string): Promise<SwapOrder> {
    const o = await this.must(id);
    if (o.status !== "main_locked") throw new Error(`order ${id} main not locked (status ${o.status})`);
  ...
  async claimCounter(id: string, preimageHex: string): Promise<{ order: SwapOrder; updateId: string }> {
    const o = await this.must(id);
    if (o.status !== "counter_locked") throw new Error(`order ${id} counter not locked (status ${o.status})`);
    ...
    const rel = await releaseCbtcOnReveal(
      this.canton,
      { receiverParty: o.userCantonParty, amountBtc: o.cbtcAmount, swapId: o.id, solverTimelock: o.solverTimelock },
    ...
    o.revealedPreimage = ("0x" + (preimageHex.startsWith("0x") ? preimageHex.slice(2) : preimageHex)) as Hex;
  ...
  async recordMainClaim(id: string, mainClaimTx: Hex): Promise<SwapOrder> {
    const o = await this.must(id);
    if (o.status !== "counter_claimed") throw new Error(`order ${id} counter not claimed (status ${o.status})`);
