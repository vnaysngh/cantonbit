...
async function main() {
  ...
  assert.ok(health.agent?.startsWith("0x"), "health should expose the agent address");
  ...
  const quoteRes = await fetch(`${base}/quote`, {
    ...
    body: JSON.stringify({ user: env.agentAccount.address, wbtcAmount: "10000", cantonParty: RECIPIENT }),
  });
  ...
  assert.ok(/^0x[0-9a-f]{64}$/i.test(quote.orderId), "orderId should be bytes32");
  assert.ok(quote.permit2?.domain && quote.permit2?.types && quote.permit2?.message, "permit2 typed data present");
  ...
  assert.equal(quote.cbtcAmount, "10000", "0 bps fee → 1:1 (cbtcOut == wbtcIn)");
  ...
  assert.ok(verifyCantonParty(RECIPIENT, committed), "quoted recipient must match keccak256(party)");
  ...
  const over = await fetch(`${base}/quote`, {
    method: "POST", headers: { "content-type": "application/json" },
    body: JSON.stringify({ user: env.agentAccount.address, wbtcAmount: "999999999", cantonParty: RECIPIENT }),
  });
  ...
  const badParty = await fetch(`${base}/quote`, {
    method: "POST", headers: { "content-type": "application/json" },
    body: JSON.stringify({ user: env.agentAccount.address, wbtcAmount: "10000", cantonParty: "not-a-party" }),
  });
...
main().catch((e) => { console.error("\n✗ API SMOKE FAILED:", e instanceof Error ? e.message : e); process.exit(1); });
