...
async function main() {
  ...
  assert.equal(
    ...
    "mainnet",
    "SWAP_NETWORK must be mainnet"
  );
  assert.equal(process.env.ALLOW_MAINNET, "true", "ALLOW_MAINNET must be true");
  assert.equal(
    (process.env.EVM_CHAIN ?? "").toLowerCase(),
    "arbitrum",
    "EVM_CHAIN must be arbitrum"
  );
  ...
  const canton = new CantonClient(
    {
      ...
      instrumentId: {
        admin: env("CANTON_ADMIN_PARTY"),
        id: process.env.CANTON_INSTRUMENT_ID ?? "CBTC"
      },
  ...
  const waitBal = async (a: Address, want: bigint, label: string) => {
    for (let i = 0; i < 40; i++) {
      if ((await balOf(a)) === want) return;
      await sleep(1500);
    }
    throw new Error(`${label}: ${a} never reached ${want}`);
  };

  const summary: Record<string, string> = {};
  console.log(
    `MAINNET swap: ${Number(WBTC_LOCK) / 1e8} WBTC (Arbitrum) → ${CBTC_BTC} CBTC (Canton mainnet)`
  );
  ...
  console.log(
    autoAccepted
      ? "(auto-accepted)"
      : `offerId: ${offerContractId} — ACCEPT IT NOW in the receiving wallet`
  );
  ...
  if (autoAccepted) {
    ...
    console.log(
      `>>> ACCEPT the incoming ${CBTC_BTC} CBTC transfer in the receiving wallet <<<`
    );
    ...
    fillTimestamp = now;
    for (let i = 0; i < 120; i++) {
      const accepted = await canton
        .isDeliveryAccepted(inputHoldingCids)
        .catch(() => false);
      if (accepted) {
  ...
  if (outcome.kind === "finalised") {
    summary["5. attest"] = outcome.attestTxHash ?? "-";
    summary["5. finalise"] = outcome.finaliseTxHash ?? "-";
  }

  console.log(`\n\n========== ✓ MAINNET SWAP PASSED ==========`);
  console.log(
    `  ${Number(WBTC_LOCK) / 1e8} WBTC (Arbitrum) → ${CBTC_BTC} CBTC (Canton mainnet)`
  );
...
main().catch((e) => {
  console.error("\n✗ MAINNET E2E FAILED:", e instanceof Error ? e.message : e);
  process.exit(1);
});
