...
const PARTY =
  "rcv::1220beefbeefbeefbeefbeefbeefbeefbeefbeefbeefbeefbeefbeefbeefbeef";
  ...
  return {
    ...
    allocate: async () => {
      ...
      return {
        ...
        lockedHoldingCids: ["h1"]
      };
    },
    executeAllocation: async () => {
      opts.onExecute?.();
      return { updateId: "exec-u" };
    },
    withdrawAllocation: async () => {
      opts.onWithdraw?.();
      return { updateId: "wd-u" };
    }
...
async function seedSeenNoParty(
  store: OrderStore,
  o: SerializedOrder,
  remember: boolean
): Promise<Hex> {
...
test("C4: two concurrent startDelivery on the SAME order — does the status guard prevent double-delivery?", async () => {
  ...
  const holdings: HoldingLite[] = [
    { contractId: "h1", amount: "1", createdEventBlob: "b", locked: false }
  ];
  let createCalls = 0;
  const canton = mockCanton({
    floatSats: 100_000n,
    holdings,
    onCreate: () => {
      createCalls++;
  ...
  const [a, b] = await Promise.all([
    startDelivery(store, canton, id, params).catch((e) => ({
      kind: "error",
      reason: String(e)
    })),
    startDelivery(store, canton, id, params).catch((e) => ({
  ...
  console.log(
    `[C4] createOffer called ${createCalls}x for one order; outcomes: ${(a as any).kind}, ${(b as any).kind}`
  );
  const rec = (await store.get(id))!;
  assert.ok(
    ["delivering", "delivered"].includes(rec.status),
    `final status should be delivering/delivered, got ${rec.status}`
  );
  ...
  assert.equal(
    createCalls,
    1,
    `concurrency claim must prevent double-delivery (createOffer called ${createCalls}x)`
  );
  ...
  assert.deepEqual(
    kinds,
    ["delivering", "skipped"],
    `one delivers, one skips; got ${kinds}`
  );
  console.log(
    "[C4] ✓ FIXED: atomic claim prevented double-delivery (1 createOffer, other skipped)."
  );
});
...
test("RECOVERY: an order whose party was remembered at quote time IS delivered", async () => {
  ...
  assert.equal(
    await store.recallParty(id),
    PARTY,
    "precondition: party was remembered (durable before openFor)"
  );

  const out = await startDelivery(store, canton, id, params);

  assert.ok(
    ["delivering", "delivered"].includes((out as any).kind),
    `must deliver, got ${(out as any).kind}`
  );
  assert.equal(createCalls, 1, "the CBTC offer must be created");
  assert.equal(
    (await store.get(id))!.cantonParty,
    PARTY,
    "party present on the record for delivery"
  );
  console.log(
    "[RECOVERY] ✓ a remembered party is durable + delivery proceeds (Postgres: one atomic row)."
  );
});
...
test("RECOVERY: an order with NO cantonParty AND none remembered is SKIPPED (not delivered blind)", async () => {
  ...
  assert.equal(
    out.kind,
    "skipped",
    "must NOT deliver without a party preimage from anywhere"
  );
  assert.equal(
    createCalls,
    0,
    "no CBTC offer when the party can't be recovered"
  );
  console.log(
    "[RECOVERY] ✓ no blind delivery when the party is truly unknown."
  );
});
...
test("RECOVERY: a POISONED recovery map (wrong party) is REJECTED by the recipient-hash check", async () => {
  ...
  assert.equal(
    out.kind,
    "failed",
    "a recovered party that doesn't hash to the commitment must be rejected"
  );
  assert.equal(
    createCalls,
    0,
    "no CBTC offer when the recovered party is wrong (no redirect)"
  );
  console.log(
    "[RECOVERY] ✓ poisoned recovery map can't redirect CBTC — recipient-hash check holds."
  );
});
...
test("PRE-FLIGHT: aborts delivery when WBTC is NOT securely claimable", async () => {
  ...
  assert.equal(
    out.kind,
    "skipped",
    "must NOT deliver when WBTC isn't claimable"
  );
  assert.equal(
    createCalls,
    0,
    "must NOT create the CBTC offer when pre-flight fails"
  );
  // order stays 'seen' (not delivering) so nothing is half-done.
  assert.equal((await store.get(id))!.status, "seen");
  console.log(
    "[PRE-FLIGHT] ✓ delivery aborted; no CBTC sent when WBTC not guaranteed."
  );
});
...
test("PRE-FLIGHT: proceeds when WBTC IS claimable", async () => {
  ...
  assert.ok(
    ["delivering", "delivered"].includes((out as any).kind),
    "delivers when WBTC is claimable"
  );
});
...
test("C3: in-flight cap skips a new delivery once the ceiling is reached", async () => {
  ...
  const out = await startDelivery(store, canton, id, {
    ...params,
    maxInflightSats: 1500n
  });
  assert.equal(
    out.kind,
    "skipped",
    "must skip when in-flight cap would be exceeded"
  );
  console.log(
    "[C3] ✓ in-flight cap enforced: new delivery skipped above the ceiling."
  );
});
...
test("C3b: under the cap, delivery proceeds", async () => {
  ...
  const out = await startDelivery(store, canton, id, {
    ...params,
    maxInflightSats: 5000n
  });
  assert.ok(
    ["delivering", "delivered"].includes((out as any).kind),
    "should proceed under the cap"
  );
});
...
test("C5: after a crash, a 'seen' order with a passed fillDeadline is NOT delivered (can't fill late)", async () => {
  ...
  assert.equal(
    out.kind,
    "skipped",
    "must not deliver an order past its fillDeadline after a crash"
  );
  console.log(
    "[C5] ✓ a stuck 'seen' order past fillDeadline is skipped (won't deliver late) — its WBTC refunds via the escrow timeout."
  );
});
...
test("C5b: a record round-trips through the store (insert → update → read back)", async () => {
  ...
  assert.equal(
    rec?.status,
    "delivering",
    "status update must be readable back"
  );
  assert.equal(rec?.cantonDeliveryRef, "offer1", "metadata must round-trip");
  console.log(
    "[C5b] ✓ record round-trips through the store (persistence now via Postgres)."
  );
});
...
test("C6: per-user cap SKIPS a user's 2nd order once their in-flight value hits the ceiling", async () => {
  ...
  const out = await startDelivery(store, canton, a2, {
    ...params,
    perUserInflightCapSats: 1500n
  });
  assert.equal(
    out.kind,
    "skipped",
    "user A's 2nd order must skip once over their per-user cap"
  );
  assert.match(
    (out as any).reason,
    /per-user in-flight cap/,
    "skip reason must cite the per-user cap"
  );
  console.log(
    "[C6] ✓ per-user cap enforced: a single user can't exceed their in-flight ceiling."
  );
});
...
test("C6b: a DIFFERENT user is NOT starved by user A's in-flight orders (cap is per-user, not global)", async () => {
  ...
  const out = await startDelivery(store, canton, b1, {
    ...params,
    perUserInflightCapSats: 1500n
  });
  assert.ok(
    ["delivering", "delivered"].includes((out as any).kind),
    "user B must NOT be blocked by user A's usage"
  );
  console.log(
    "[C6b] ✓ cap is per-user: B is not starved by A — no false cross-user blocking."
  );
});
...
test("C6c: under their own cap, a user's 2nd order proceeds", async () => {
  ...
  const out = await startDelivery(store, canton, a2, {
    ...params,
    perUserInflightCapSats: 5000n
  });
  assert.ok(
    ["delivering", "delivered"].includes((out as any).kind),
    "A's 2nd order proceeds while under their cap"
  );
...
test("C6d: per-user cap counts delivering + delivered, but NOT finalised/failed/refunded", async () => {
  ...
  const done1 = await seedSeen(store, orderForUser("100000", USER_A));
  await store.update(done1, { status: "finalised" });
  const done2 = await seedSeen(store, orderForUser("100000", USER_A));
  await store.update(done2, { status: "refunded" });
  const done3 = await seedSeen(store, orderForUser("100000", USER_A));
  await store.update(done3, { status: "failed" });
  // Plus one genuinely in-flight order worth 1000.
  const live = await seedSeen(store, orderForUser("1000", USER_A));
  await store.update(live, { status: "delivered" });
  ...
  const out = await startDelivery(store, canton, a, {
    ...params,
    perUserInflightCapSats: 1500n
  });
  assert.equal(
    out.kind,
    "skipped",
    "only delivering/delivered count; the finalised/refunded/failed 100k each must NOT inflate the sum"
  );
  console.log(
    "[C6d] ✓ cap counts ONLY in-flight (delivering/delivered); settled/failed/refunded excluded."
  );
});
...
test("C6e: no per-user cap configured → unlimited (opt-in, off by default)", async () => {
  ...
  assert.ok(
    ["delivering", "delivered"].includes((out as any).kind),
    "with no cap set, orders are unlimited"
  );
  console.log(
    "[C6e] ✓ cap is opt-in: unset → no per-user limit (back-compat)."
  );
});
 ...
 */
function debitingCanton(
  start: bigint,
  onSpend: (sats: bigint) => void
): { canton: CantonClient; float: () => bigint } {
  let floatSats = start;
  const holdings: HoldingLite[] = [
    { contractId: "h1", amount: "1", createdEventBlob: "b", locked: false }
  ];
...
test("C7: many users delivering concurrently never over-deliver the shared float", async () => {
  ...
  let delivered = 0n;
  const { canton, float } = debitingCanton(5000n, (s) => {
    delivered += s;
  });
  ...
  for (const id of ids) {
    ...
    else if (/insufficient CBTC float/.test((out as any).reason ?? ""))
      failedFloat++;
  }

  console.log(
    `[C7] ${succeeded} delivered, ${failedFloat} rejected (insufficient float); total delivered=${delivered} sats, float started=5000`
  );
  assert.ok(
    delivered <= 5000n,
    `NEVER over-deliver the float: delivered ${delivered} > 5000`
  );
  assert.equal(
    succeeded,
    5,
    "exactly 5 orders fit in a 5000-sat float at 1000 each"
  );
  assert.equal(
    failedFloat,
    5,
    "the other 5 must be cleanly rejected, not over-delivered"
  );
  assert.ok(float() >= 0n, `float must never go negative (got ${float()})`);
  console.log(
    "[C7] ✓ multi-user: shared float never over-delivered; surplus orders rejected cleanly."
  );
});
...
test("C7b: per-user cap + multi-user — one greedy user can't starve the float; others still served", async () => {
  ...
  const aIds = await Promise.all(
    Array.from({ length: 5 }, () =>
      seedSeen(store, orderForUser("1000", USER_A))
    )
  );
  ...
  console.log(
    `[C7b] greedy user A got ${aSucceeded}/5 (cap 2000=2 orders); honest user B served: ${(bOut as any).kind}`
  );
  assert.equal(
    aSucceeded,
    2,
    "greedy user A is capped at 2 in-flight (2000 sats), not all 5"
  );
  assert.ok(
    ["delivering", "delivered"].includes((bOut as any).kind),
    "honest user B is STILL served — not starved by A"
  );
  console.log(
    "[C7b] ✓ per-user cap protects the float: greedy user throttled, honest user served."
  );
});
