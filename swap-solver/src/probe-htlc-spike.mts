...
const PREIMAGE_HEX = "7468652d63726f73732d636861696e2d7365637265742d333262797465732121";
const HASHLOCK = "94277b389401042e35f8709846050797955e2b321bee500555c2fbdc2f4e9903"; // keccak256 of it

function reqEnv(k: string): string { const v = process.env[k]; if (!v) throw new Error(`missing env ${k}`); return v; }
const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

async function getJwt(): Promise<string> {
  const body = new URLSearchParams({ grant_type: "client_credentials", client_id: "validator-devnet-m2m", client_secret: SECRET, scope: SCOPE });
  const r = await fetch(TOKEN_URL, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body });
  if (!r.ok) throw new Error(`token failed ${r.status}: ${await r.text()}`);
  return ((await r.json()) as { access_token: string }).access_token;
}

async function submit(jwt: string, commands: unknown[], disclosed: unknown[] = []): Promise<any> {
  const commandId = randomUUID();
  const r = await fetch(`${LEDGER}/v2/commands/submit-and-wait-for-transaction-tree`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${jwt}` },
    body: JSON.stringify({
      applicationId: "cbtc-htlc-spike", workflowId: `spike-${commandId}`, commandId,
      actAs: [SOLVER], readAs: [SOLVER], commands, disclosedContracts: disclosed,
    }),
  });
...
async function allocChoiceContext(jwt: string, allocationCid: string, kind: string) {
  ...
  const r = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ meta: {} }) });
  if (!r.ok) throw new Error(`choice-context ${kind} failed (${r.status}): ${await r.text()}`);
  const ctx = (await r.json()) as { choiceContextData: unknown; disclosedContracts: any[] };
  return { data: ctx.choiceContextData, disclosed: (ctx.disclosedContracts ?? []).map((d) => ({ ...d, synchronizerId: d.synchronizerId ?? "" })) };
}
...
async function main() {
  ...
  const canton = new CantonClient(
    ...
    { tokenUrl: TOKEN_URL, clientId: "validator-devnet-m2m", clientSecret: SECRET, scope: SCOPE },
  );
  ...
  const { allocationCid } = await canton.allocate({
    receiverParty: RECEIVER, amountBtc: AMOUNT, inputHoldings: holdings,
  ...
  try {
    ...
    const createRes = await submit(jwt, [{
      CreateCommand: {
        templateId: HTLC_TID,
        createArguments: {
          locker: SOLVER, receiver: RECEIVER, executor: SOLVER,
          allocationCid, hashLock: HASHLOCK, unlockTime,
        },
      },
    }]);
    ...
    const ctx = await allocChoiceContext(jwt, allocationCid, "execute-transfer");

    // 5. THE DECISIVE EXERCISE — Claim with the preimage → fires Allocation_ExecuteTransfer
    console.log(`[4] exercising HtlcLock.Claim(preimage) as solver — THE TEST…`);
    await submit(jwt, [{
      ExerciseCommand: {
        templateId: HTLC_TID,
        contractId: htlcCid,
    ...
    console.log(`    The custom Daml HTLC drives a real cBTC release. MECHANISM PROVEN.`);

    await sleep(2000);
    const float1 = await canton.getFloatSats();
    console.log(`[5] float after: ${Number(float1)/1e8} cBTC (self-swap → net-zero expected)`);
  } catch (e) {
    console.error(`\n    ❌ FAILED at the decisive step: ${e instanceof Error ? e.message : e}`);
    console.error(`    (If DAML_AUTHORIZATION_ERROR → the registry still demands receiver/other auth.)`);
  } finally {
    if (!done) {
      console.error(`\n[safety] attempting Allocation_Withdraw to recover the locked cBTC…`);
      try { const { updateId } = await canton.withdrawAllocation(allocationCid); console.error(`[safety] ✓ withdrawn (${updateId.slice(0,16)}…) — cBTC recovered.`); }
...
function findCreated(txTreeRes: any, templateSuffix: string): string {
  ...
  for (const ev of Object.values(events) as any[]) {
    ...
    if (c?.contractId && (c.templateId ?? "").includes("HtlcLock")) return c.contractId;
  }
...
main().catch((e) => { console.error("\n[spike] FATAL:", e instanceof Error ? e.message : e); process.exit(1); });
