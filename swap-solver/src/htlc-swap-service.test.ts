...
import { HtlcSwapService, InMemorySwapStore, type SwapOrder } from "./htlc-swap-service.js";
...
function fakeCanton(): any {
  return {
    ...
    async createOffer(p: any) { (this.released as any[]).push(p); return { updateId: "u-" + p.commandId, offerContractId: "off", autoAccepted: true, inputHoldingCids: [] }; },
  };
}
...
test("full happy path: open→accepted→main_locked→counter_locked→counter_claimed→main_claimed", async () => {
  ...
  assert.equal(canton.released.length, 1, "cBTC was released exactly once, on the reveal");
...
test("claim-counter REJECTS a wrong preimage — cBTC is NOT released (the gate)", async () => {
  ...
  const wrong = toHex(new TextEncoder().encode("wrong-secret-wrong-secret-wrong!")).slice(2);
  await assert.rejects(() => svc.claimCounter("swap-1", wrong), /invalid preimage/);
  assert.equal(canton.released.length, 0, "NO cBTC released on a bad preimage");
  assert.equal((await store.get("swap-1"))!.status, "counter_locked", "still locked");
});
...
test("cannot skip steps: claim-counter before counter_locked fails", async () => {
  ...
  await svc.createOrder(baseOrder());
  await assert.rejects(() => svc.claimCounter("swap-1", PREIMAGE_HEX), /counter not locked/);
});
