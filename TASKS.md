...
our node. Reference production patterns confirm Loop custody mode is
...
- [x] **Canton → EVM (reverse) — BUILT (email users, fully trustless), needs browser
 test.** HTLC reverse-direction design (docs/canton-to-evm-design.md): backend locks the
...
- [x] **LOOP SELLERS (canton-to-evm, external wallet) — Variant A custody,
 ...
 settles only because the CONTRACT aggregates authorities — email-only. The
...
- [ ] Fee model: 1% per side in the sent token (optional but standard).
...
- Loop custody mode (derived from reference production patterns, 2026-06-11) =
